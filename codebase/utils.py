...
def visualize_samples(vae,n_samples,grid_sizes,figsize):
    x_sample = vae.sample_x(n_samples)
    x_sample = x_sample.reshape(n_samples,28,28).detach().numpy()

    fig,axs = plt.subplots(grid_sizes[0],grid_sizes[1],figsize=figsize)
    axs = axs.flatten()
    for i in range(200):
        axs[i].imshow(x_sample[i],cmap = cm.Greys_r)
        axs[i].axis("off")
    fig.tight_layout()
    fig.savefig("MNIST_decoder_samples.png",dpi=600)

def visualize_2d_mappings(vae,x,y):
    fig, ax = plt.subplots()
    m,v = vae.enc(x.reshape(-1,784))
    sc = ax.scatter(m[:,0].detach(),m[:,1].detach(),c=y,alpha=0.1,cmap='hsv')
    cbar = plt.colorbar(sc)
    cbar.solids.set(alpha=1)

    ax.set_xlabel(r"$z_1$")
    ax.set_ylabel(r"$z_2$")
    ax.set_title(r"Distribution of Numbers in Latent Space")
    fig.tight_layout()
    fig.savefig("distribution_latent.png",dpi=600)
